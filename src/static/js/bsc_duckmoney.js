...
const DUCK_CHEF_ABI = [
  {
    inputs: [
      ...
      {internalType: 'uint256', name: '_duckPerBlock', type: 'uint256'},
  ...
  {
    anonymous: false,
    inputs: [{indexed: false, internalType: 'uint256', name: 'cut', type: 'uint256'}],
    name: 'Scrooge',
    type: 'event',
  },
  {
  ...
  {
    inputs: [{internalType: 'uint256', name: 'cut', type: 'uint256'}],
    name: 'scrooge',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  ...
  {
    inputs: [{internalType: 'uint256', name: '_duckPerBlock', type: 'uint256'}],
    name: 'updateEmissionRate',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
...
async function main() {
  ...
  const rewardsPerWeek = (((await DUCK_CHEF.duckPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadDuckChefContract(
    App,
    tokens,
    prices,
    DUCK_CHEF,
...
async function loadDuckChefContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(async x =>
      x == 0 ? null : await getBscPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
