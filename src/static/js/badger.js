...
const getRewards = async (
  ...
  if (userStaked > 0) {
    if (index == 0) {
      _print(
        `You are staking ${userStaked.toFixed(12)} ${settToken.symbol} (${userUnderlyingStaked.toFixed(12)} ${
          lpToken.symbol
...
async function main() {
  ...
  for (const p of pools) await printPool(App, tokens, prices, p, sharesPerFragment)
