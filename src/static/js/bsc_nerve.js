...
const NERVE_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract NerveToken', name: '_nerve', type: 'address'},
      {internalType: 'address', name: '_devaddr', type: 'address'},
      {internalType: 'uint256', name: '_nervePerBlock', type: 'uint256'},
  ...
  {inputs: [], name: 'massUpdatePools', outputs: [], stateMutability: 'nonpayable', type: 'function'},
  {
    inputs: [],
    name: 'nerve',
    outputs: [{internalType: 'contract NerveToken', name: '', type: 'address'}],
    stateMutability: 'view',
    type: 'function',
  },
  {
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accNervePerShare', type: 'uint256'},
    ],
    stateMutability: 'view',
    type: 'function',
  },
  ...
  {
    inputs: [{internalType: 'uint256', name: '_nervePerBlock', type: 'uint256'}],
...
async function loadNerveChefContract(
  App,
  tokens,
  prices,
  chef,
...
async function main() {
  ...
  const rewardsPerWeek = (((await NERVE_CHEF.nervePerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  _print(
    '*** The APR listed is for immediate rewards, which is 1/3rd of the total rewards. The extra 2/3rds vest over 6 months. ***\n'
  )

  await loadNerveChefContract(
    App,
    tokens,
    prices,
    NERVE_CHEF,
