...
const ALPACA_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract AlpacaToken', name: '_alpaca', type: 'address'},
      {internalType: 'address', name: '_devaddr', type: 'address'},
      {internalType: 'uint256', name: '_alpacaPerBlock', type: 'uint256'},
      {internalType: 'uint256', name: '_startBlock', type: 'uint256'},
      {internalType: 'uint256', name: '_bonusLockupBps', type: 'uint256'},
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accAlpacaPerShare', type: 'uint256'},
  ...
  {
    inputs: [{internalType: 'uint256', name: '_alpacaPerBlock', type: 'uint256'}],
...
async function loadAlpacaBscChefContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getAlpacaPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
...
async function main() {
  ...
  const rewardsPerWeek = ((((await ALPACA_CHEF.alpacaPerBlock()) / 1e18) * 604800) / 3) * 7 //bonus multiplier
  ...
  await loadAlpacaBscChefContract(
    App,
    tokens,
    prices,
    ALPACA_CHEF,
