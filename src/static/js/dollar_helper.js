...
const loadDAO = async (
  ...
  var newPrices = await lookUpTokenPrices(
    uniPool.tokens.filter(
      t =>
        t.toLowerCase() != '0x5cf9242493be1411b93d064ca2e468961bbb5924' &&
        t.toLowerCase() != '0xf0e3543744afced8042131582f2a19b6aeb82794'
    )
  ) //Exceptions for ESG and VTD due to coingecko bug
  ...
  if (DAO.callStatic['nextEpochStart'])
    _print(`Next Epoch at: ${new Date((await DAO.nextEpochStart()) * 1000).toString()}`)
  ...
  _print(
    `You have ${staged.toFixed(decimals)} staged ${dollar}, $${formatMoney(staged * zaiPrice)}, ${(
      (staged / totalStaged) *
      100
    ).toFixed(4)}% of the pool`
  )
  _print(
    `You have ${bonded.toFixed(decimals)} bonded ${dollar}, $${formatMoney(bonded * zaiPrice)}, ${(
      (bonded / totalBonded) *
      100
    ).toFixed(4)}% of the pool`
  )
...
async function loadEmptySetLP(
  ...
  const rewarded =
    (rewardTicker == 'ESG' || rewardTicker == 'ESB'
      ...
      : await LP.balanceOfRewarded(App.YOUR_ADDRESS)) / 1e18
  const status = (rewardTicker == 'ESG' || rewardTicker == 'ESB'
...
const esd_claim = async function(LP, App) {
  ...
  if (claimable > 0) {
    showLoading()
    LP.connect(signer)
      .claim(claimable, {gasLimit: 500000})
      .then(function(t) {
        return App.provider.waitForTransaction(t.hash)
      })
      .catch(function() {
...
const esd_provide = async (LP, App, baseTokenAddress, extraParam) => {
  ...
  if (rewarded > 0) {
    showLoading()
    LP.connect(signer)
      .provide(rewarded, {gasLimit: 500000})
      .then(function(t) {
        return App.provider.waitForTransaction(t.hash)
      })
      .catch(function() {
...
async function printDaoUnbonds(provider, DAO, epoch, fluidEpochs, epochTimeSec, price, displayDecimals) {
  ...
  for (let i = 0; i < fluidEpochs; i++) {
    ...
    _print(
      `Unbonding at epoch ${epoch + i}: ${unbonding.toFixed(displayDecimals ?? 2)} ($${formatMoney(unbonding * price)})`
    )
  }
}

async function printLPUnbonds(
  provider,
  LP,
  epoch,
  fluidEpochs,
  ...
  for (let i = 0; i < fluidEpochs; i++) {
    ...
    _print(
      `Unbonding at epoch ${epoch + i}: ${unbonding.toFixed(lpDecimals)} LPT - Cycling: ${cycling.toFixed(
        lpDecimals
      )} LPT - Claimable: ${claimable.toFixed(displayDecimals ?? 2)} ${rewardTicker} ($${formatMoney(
        claimable * price
      )})`
    )
  }
...
async function calculateDollarAPR(
  DAO,
  parameters,
  twap,
  dollarPrice,
  ...
  if (daoRewards > 0) {
    ...
    _print(
      `DAO APY: Day ${bondedReturn.toFixed(2)}% Week ${(bondedReturn * 7).toFixed(2)}% Year ${(
        bondedReturn * 365
      ).toFixed(2)}%`
    )
...
async function loadDollar(contractInfo, calcPrice, getEpochPeriod, getTwap) {
  ...
  const [epoch, uniPrices, totalBonded] = await loadDAO(
    App,
    DAO,
    DOLLAR,
    contractInfo.UniswapLP.address,
    poolInfo?.address,
    tokens,
    prices,
    params.DaoLockupPeriods,
    false,
    contractInfo.Dollar.displayDecimals,
    epochPeriod,
    contractInfo.StakingPool?.address
  )

  const dollarPrice = getParameterCaseInsensitive(prices, DOLLAR.address).usd

  const baseTokenAddress = Object.entries(tokens).filter(
    ([k]) => k.toLowerCase() !== DOLLAR.address.toLowerCase()
  )[0][1].address

  let LP
  if (poolInfo) {
    LP = new ethers.Contract(poolInfo.address, poolInfo.abi, App.provider)
    await loadEmptySetLP(
      App,
      LP,
      contractInfo.UniswapLP.address,
