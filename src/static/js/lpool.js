...
async function main() {
  ...
  const LPOOL_STAKE_FARMING_ABI = [
    {
      inputs: [
        {internalType: 'contract LaunchPoolToken', name: '_lpt', type: 'address'},
        {internalType: 'uint256', name: '_maxLPTAvailableForFarming', type: 'uint256'},
    ...
    },
    {
      inputs: [
        ...
        {internalType: 'uint256', name: '_maxStakingAmountPerUser', type: 'uint256'},
    ...
    },
    {
      inputs: [],
      name: 'lpt',
      outputs: [{internalType: 'contract LaunchPoolToken', name: '', type: 'address'}],
      stateMutability: 'view',
      type: 'function',
    },
    {
    ...
    },
    {
      ...
      outputs: [
        ...
        {internalType: 'uint256', name: 'accLptPerShare', type: 'uint256'},
        {internalType: 'uint256', name: 'maxStakingAmountPerUser', type: 'uint256'},
      ],
      stateMutability: 'view',
      type: 'function',
    },
  ...
  const rewardsPerWeek = (((await LPOOL_CHEF.lptPerBlock()) / 1e18) * 604800) / 13.5

  await loadLpoolChefContract(
    App,
    LPOOL_CHEF,
    LPOOL_CHEF_ADDR,
    LPOOL_STAKE_FARMING_ABI,
    rewardTokenTicker,
    'lpt',
    null,
    rewardsPerWeek,
...
async function loadLpoolChefContract(
  App,
  chef,
  chefAddress,
  chefAbi,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getLpoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
...
async function getLpoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  if (poolInfo.allocPoint == 0 || /^LP\d+$/.test(poolToken.symbol)) {
    //LP1 etc
    return {
