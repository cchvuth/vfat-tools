...
const ALCX_POOL_ABI = [
  {
    inputs: [
      {internalType: 'contract IMintableERC20', name: '_reward', type: 'address'},
  ...
  {
    inputs: [{internalType: 'address', name: '_pendingGovernance', type: 'address'}],
  ...
  {
    inputs: [{internalType: 'uint256[]', name: '_rewardWeights', type: 'uint256[]'}],
...
async function getAlcxPoolInfo(App, pool, poolIndex) {
  const [token, rewardRate, totalDeposited, userStaked, userUnclaimed] = await App.ethcallProvider.all([
    pool.getPoolToken(poolIndex),
    pool.getPoolRewardRate(poolIndex),
    pool.getPoolTotalDeposited(poolIndex),
    pool.getStakeTotalDeposited(App.YOUR_ADDRESS, poolIndex),
...
function printAlcxContractLinks(
  App,
  alcxAbi,
  alcxAddr,
  poolIndex,
...
async function main() {
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount / 1).keys()].map(async x => await getAlcxPoolInfo(App, ALCX_POOL, x))
  )
