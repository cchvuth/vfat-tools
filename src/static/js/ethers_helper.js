...
async function getUniPool(app, pool, poolAddress, stakingAddress) {
  ...
  const [decimals, token0, token1, symbol, name, totalSupply, staked, unstaked] = await app.ethcallProvider.all(calls)
  let q0, q1, is1inch
  try {
...
async function getVault(app, vault, address, stakingAddress) {
  ...
  const [decimals, underlying, name, symbol, totalSupply, staked, unstaked, balance] = await app.ethcallProvider.all(
    calls
  )
  ...
  return {
    ...
    print_price(chain = 'eth') {
      const poolUrl = pool.is1inch
        ...
        ? `http://app.sushi.com/pair/${pool.address}`
      ...
      const tvlEle = document.getElementById('total-tvl')
      if (tvlEle) tvlEle.innerHTML = formatMoney(Number(tvlEle.innerHTML.split(',').join('')) + tvl)
...
function getWrapPrices(tokens, prices, pool) {
  ...
  if (wrappedToken.token0 != null) {
    ...
    const poolUrl = pool.is1inch
      ? 'https://1inch.exchange/#/dao/pools'
      : pool.symbol.includes('SLP')
      ? `http://app.sushi.com/pair/${wrappedToken.address}`
...
async function loadChefContract(
  ...
  if (deathPoolIndices) {
    ...
    deathPoolIndices
      .map(i => poolInfos[i])
      .map(poolInfo => (poolInfo.poolToken ? getPoolPrices(tokens, prices, poolInfo.poolToken, 'eth') : undefined))
  }
...
async function loadBoardroom(
  ...
  _print(
    `You are staking ${userStaked} ${stakeTicker} ($${formatMoney(userStakedUsd)}), ${userPct.toFixed(2)}% of the pool.`
  )
...
async function printSynthetixPool(App, info, chain = 'eth') {
  ...
  switch (chain) {
    ...
    case 'matic':
      _print(
        `<a target="_blank" href="https://explorer-mainnet.maticvigil.com/address/${info.stakingAddress}#code">Matic Explorer</a>`
      )
      break
  }
  ...
  _print_link(
    `Claim ${info.earned.toFixed(6)} ${info.rewardTokenTicker} ($${formatMoney(info.earned * info.rewardTokenPrice)})`,
    claim
  )
  if (info.stakeTokenTicker != 'ETH') {
    _print_link(`Revoke (set approval to 0)`, revoke)
...
async function getNewPricesAndTokens(App, tokens, prices, newAddresses, stakingAddress) {
  var newPriceAddresses = newAddresses.filter(x => !getParameterCaseInsensitive(prices, x))
  var newPrices = await lookUpTokenPrices(newPriceAddresses)
  for (const key in newPrices) {
    if (newPrices[key]) prices[key] = newPrices[key]
  }
  var newTokenAddresses = newAddresses.filter(x => !getParameterCaseInsensitive(tokens, x))
  for (const address of newTokenAddresses) {
    tokens[address] = await getToken(App, address, stakingAddress)
  }
}
