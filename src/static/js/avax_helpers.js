...
async function getAvaxPrices() {
  ...
  const prices = {}
  for (const bt of avaxTokens) if (idPrices[bt.id]) prices[bt.contract] = idPrices[bt.id]
  return prices
}
...
async function loadAvaxSynthetixPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
...
async function loadAvaxSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadAvaxSynthetixPoolInfo(
    App,
    tokens,
    prices,
    abi,
...
async function loadAvaxBasisFork(data) {
  ...
  let p1 = await loadAvaxSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
  ...
  if (data.SharePool2) {
    let p3 = await loadAvaxSynthetixPool(
      App,
      tokens,
      prices,
      data.PoolABI,
  ...
  let p2 = await loadAvaxSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
...
async function loadAvaxChefContract(
  App,
  tokens,
  prices,
  chef,
  ...
  if (deathPoolIndices) {
    ...
    deathPoolIndices
      .map(i => poolInfos[i])
      .map(poolInfo => (poolInfo.poolToken ? getPoolPrices(tokens, prices, poolInfo.poolToken, 'avax') : undefined))
  }
...
async function loadMultipleAvaxSynthetixPoolsSequential(App, tokens, prices, pools) {
  ...
  for (const p of pools) {
    let res = await loadAvaxSynthetixPool(
      App,
      tokens,
      prices,
      p.abi,
