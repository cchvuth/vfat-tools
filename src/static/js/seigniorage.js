...
const getDollar = async (App, basePrices, v) => {
  ...
  const status =
    epoch <= (v.Parameters?.BootstrappingPeriod ?? 0) ? 'Bootstrap' : twap > 1 ? 'Expansion' : 'Contraction'
  return {
    page: v.Page,
...
const getBasisFork = async (App, basePrices, v) => {
  try {
    const getPrice = (lp, token) =>
      getPriceFromUniswap(App, basePrices, lp, token, v.CashPool.baseCoin, v.CashPool.baseDecimals)
...
const main = async () => {
  ...
  const basisForks = (await Promise.all(Object.entries(Basis).map(([, v]) => getBasisFork(App, basePrices, v)))).filter(
    x => x
  )
