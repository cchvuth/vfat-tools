...
const STRAT_ABI = [
  {
    inputs: [
      ...
      {internalType: 'address', name: '_rakeFinanceAddress', type: 'address'},
...
async function main() {
  ...
  const RAKEFINANCE_CHEF_ADDR = '0x7f7Bf15B9c68D23339C31652C8e860492991760d'

  const rewardTokenTicker = 'AUTO'
  const RAKEFINANCE_CHEF = new ethers.Contract(RAKEFINANCE_CHEF_ADDR, RAKEFINANCE_CHEF_ABI, App.provider)

  const rewardsPerWeek = (((await RAKEFINANCE_CHEF.AUTOPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadBscAUTOChefContract(
    App,
    tokens,
    prices,
    RAKEFINANCE_CHEF,
    RAKEFINANCE_CHEF_ADDR,
    RAKEFINANCE_CHEF_ABI,
    rewardTokenTicker,
    'AUTOv2',
    null,
    rewardsPerWeek,
...
async function loadBscAUTOChefContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(async x => await getBscAUTOPoolInfo(App, chefContract, x, pendingRewardsFunction))
  )
