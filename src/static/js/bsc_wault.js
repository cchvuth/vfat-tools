...
const WAULT_ABI = [
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accWaultPerShare', type: 'uint256'},
  ...
  {
    inputs: [{internalType: 'uint256', name: '_waultPerBlock', type: 'uint256'}],
    name: 'setWaultPerBlock',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
  {
    inputs: [{internalType: 'contract IERC20', name: '_wault', type: 'address'}],
    name: 'setWaultToken',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
...
const WAULT_LP_STAKE_ABI = [
  ...
  {
    inputs: [
      {internalType: 'contract IERC20', name: '_wault', type: 'address'},
...
async function main() {
  ...
  const rewardsPerWeek = (((await WAULT_CONTRACT.waultPerBlock()) / 1e18) * 604800) / 3
  const rewardsPerWeekLpStake = (((await WAULT_LP_STAKE_CONTRACT.waultPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadBscWaultContract(
    App,
    tokens,
    prices,
    WAULT_CONTRACT,
  ...
  await loadBscWaultLpStakeContract(
    App,
    tokens,
    prices,
    WAULT_LP_STAKE_CONTRACT,
    WAULT_ADDR_LP_STAKE,
    WAULT_LP_STAKE_ABI,
    rewardTokenTicker,
    'wault',
    null,
    rewardsPerWeekLpStake,
    'pendingRewards'
  )
...
async function loadBscWaultContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getBscWaultPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
...
async function loadBscWaultLpStakeContract(
  App,
  tokens,
  prices,
  chef,
