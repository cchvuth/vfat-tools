const xdaiTokens = [{id: 'xdai', symbol: 'xDAI', contract: '0xe91D153E0b41518A2Ce8Dd3D7944Fa863463a97d'}]

async function getXdaiPrices() {
  const idPrices = await lookUpPrices(xdaiTokens.map(x => x.id))
  const prices = {}
  for (const bt of xdaiTokens) if (idPrices[bt.id]) prices[bt.contract] = idPrices[bt.id]
  return prices
}
...
async function loadXdaiSynthetixPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
...
async function loadXdaiSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadXdaiSynthetixPoolInfo(
    App,
    tokens,
    prices,
    abi,
...
async function loadXdaiBasisFork(data) {
  ...
  let p1 = await loadXdaiSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
  ...
  if (data.SharePool2) {
    let p3 = await loadXdaiSynthetixPool(
      App,
      tokens,
      prices,
      data.PoolABI,
  ...
  let p2 = await loadXdaiSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
...
async function loadXdaiChefContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getXdaiPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
  ...
  if (deathPoolIndices) {
    ...
    deathPoolIndices
      .map(i => poolInfos[i])
      .map(poolInfo => (poolInfo.poolToken ? getPoolPrices(tokens, prices, poolInfo.poolToken, 'xdai') : undefined))
  }
...
async function loadMultipleXdaiSynthetixPoolsSequential(App, tokens, prices, pools) {
  ...
  for (const p of pools) {
    let res = await loadXdaiSynthetixPool(
      App,
      tokens,
      prices,
      p.abi,
