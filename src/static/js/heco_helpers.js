...
async function loadHecoSynthetixPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
...
async function loadHecoSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadHecoSynthetixPoolInfo(
    App,
    tokens,
    prices,
    abi,
...
async function loadHecoBasisFork(data) {
  ...
  let p1 = await loadHecoSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
  ...
  if (data.SharePool2) {
    let p3 = await loadHecoSynthetixPool(
      App,
      tokens,
      prices,
      data.PoolABI,
  ...
  let p2 = await loadHecoSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
...
async function loadHecoChefContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getHecoPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
  ...
  if (deathPoolIndices) {
    ...
    deathPoolIndices
      .map(i => poolInfos[i])
      .map(poolInfo => (poolInfo.poolToken ? getPoolPrices(tokens, prices, poolInfo.poolToken, 'heco') : undefined))
  }
...
async function getHecoPrices() {
  ...
  const prices = {}
  for (const bt of hrcTokens) if (idPrices[bt.id]) prices[bt.contract] = idPrices[bt.id]
  return prices
}
