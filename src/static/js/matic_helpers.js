...
async function loadMaticSynthetixPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
...
async function loadMaticSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadMaticSynthetixPoolInfo(
    App,
    tokens,
    prices,
    abi,
...
async function loadMaticBasisFork(data) {
  ...
  let p1 = await loadMaticSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
  ...
  if (data.SharePool2) {
    let p3 = await loadMaticSynthetixPool(
      App,
      tokens,
      prices,
      data.PoolABI,
  ...
  let p2 = await loadMaticSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
...
async function loadMaticChefContract(
  App,
  tokens,
  prices,
  chef,
...
async function getMaticPrices() {
  ...
  const prices = {}
  for (const bt of maticTokens) if (idPrices[bt.id]) prices[bt.contract] = idPrices[bt.id]
  return prices
}
...
async function loadMultipleMaticSynthetixPoolsSequential(App, tokens, prices, pools) {
  ...
  for (const p of pools) {
    let res = await loadMaticSynthetixPool(
      App,
      tokens,
      prices,
      p.abi,
