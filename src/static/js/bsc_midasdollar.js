...
const MIDAS_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'address', name: '_mds', type: 'address'},
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accMdsPerShare', type: 'uint256'},
...
const MIDAS_BOARDROOM_CHEF_ABI = [
  {
    anonymous: false,
    inputs: [
...
async function main() {
  ...
  const rewardsMidasPerWeek = (((await MIDAS_CHEF.mdsPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadMidasChefContract(
    App,
    tokens,
    prices,
    MIDAS_CHEF,
  ...
  let br = await loadBoardroom(
    App,
    prices,
    MIDAS_BOARDROOM_ADDR,
    MIDAS_BOARDROOM_ORACLE_ADDR,
    lptAddress,
    rewardTokenAddress,
    stakeTicker,
    rewardTicker,
...
async function loadMidasChefContract(
  App,
  tokens,
  prices,
  chef,
